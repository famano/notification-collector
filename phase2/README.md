...
| 種類 | 何を覚えるか | いつ渡るか |
|---|---|---|
| `profile` | 利用者そのもの。役割・担当・関心事・立場 | **常に** (どの件にも効く) |
| `preference` | こう扱ってほしい。「請求書は送らずに下書きまで」 | 話題が合うときだけ |
| `how` | 似た件をこう処理した。「この招待は API で承諾できた」 | 話題が合うときだけ |
...
- **全部は渡さない。** 1件 200 字、総数と合計の長さにも上限を置き、
  カードの文面と突き合わせて関連するものだけ渡す。上限に当たったら、
  使われていないものから落ちる
- 言い換えただけの記憶は増やさず差し替える。積むほど絞り込みが効かなくなる
