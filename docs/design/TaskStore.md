...
| | |
|---|---|
| 担う | スキーマの定義と移行 / events・tasks とその周辺表の読み書き / 同時実行の裁定 (楽観ロック・リース) / 通知と同期の同一性の計算 |
...
- **同一性の計算** (`New-EventIdentity` 〜 `Get-EventIdentityFromRow`, :442-593)
  通知側と同期側が**同じ文字列を作れなければ機能しない**。どちらかの経路に置くと、
  もう一方が別の作り方をした瞬間に静かに結ばれなくなる。両方が読む場所に集約している。
- **設定カードの生成** (`New-SetupTask`, :783)
...
| `account_id` | どのアカウントで取り込んだか。空は1人目 (`$script:PrimaryEventAccountId = '1'`, :371) |
...
「繋いだのに何も入らない」になる。`ConvertTo-AccountSourceKey` (:373) が
...
返信先のあるカードで記録を保存すると**それが送信欄に現れ、記録欄からは消えて見えた**ため (:157-164)。
...
- **`task_comments.kind`** … `NULL`/`author='user'` は利用者の指示、`report` はワーカーの報告、
  ...
  差し戻すたびに前回の報告が読めなくなっていた (:166-175)。
- **`task_comments.consumed_at`** … ワーカーが読んだ印。`Set-CommentsConsumed` に
  **読んだ id までを渡すこと** (:1234) ―― 全部を既読にすると、作業中に届いた指示が
  一度も読まれないまま消える。
...
| # | 守っていること | 担保 | 破れたときの症状 |
|---|---|---|---|
| 1 | 同じ通知を何度取り込んでもイベントは1件 | `UNIQUE(source, source_key)` + `INSERT OR IGNORE` (:404) | 再取り込みでカードが増殖する |
| 2 | 1イベントにつきカードは1枚 | `idx_tasks_event` UNIQUE + `New-Task` は `INSERT OR IGNORE` | 同上 |
| 3 | 通知と同期が同じ `dedup_key` を作る | `New-EventIdentity` に組み立てを集約 (:467) | カードが2枚立つ (症状は「重複」) |
| 4 | 束ねるのは**経路をまたぐ**1対1だけ | `Find-EventCounterpart` の `e.source <> ?` (:613) | 同じ件名のメールが続けて来ると**本物が1通消える** |
| 5 | 正は必ず同期側 | `Get-EventRank` (:602)。通知=1 / それ以外=2 | 表示用テキストだけのカードが残り、返信先を失う |
| 6 | カードの内容変更は `version` で直列化 | `Set-TaskColumn` / `Update-TaskFields` / `Set-TaskCancel` が `version+1` | 利用者の編集がワーカーの書き戻しに踏み潰される |
| 7 | `doing` のカードを握るのは1人 | `Get-NextWorkItem` のトランザクション + リース (:1301) | 2つのワーカーが同じカードで外向きの操作を二重に出す |
| 8 | 更新できる列はホワイトリスト | `$script:UpdatableFields` (:1411) | 列名が SQL に埋まるので任意列の書き換え口になる |
| 9 | 設定カードはサービスごとに1枚 | `subject_key = 'setup:{service}'` で既存を探す (:783) | 同じ権限の設定カードが言い回しの数だけ立つ |
| 10 | カードを消すと参照行も全部消える | `Remove-TaskRows` (:1179) | `FOREIGN KEY constraint failed` で**削除そのものが失敗** |

**4 と 5 は対になっている。**「同期どうしを突き合わせない」のも同じ理由で、
Gmail と Outlook の両方に届いた同じメールを束ねると、**どちらが正かを決める材料が
こちら側に無い**まま片方が消える。だから `mail` と `outlook` は別種の鍵にしてある (:455-466)。
...
Get-UntriagedEvents          triage_log に記録が無く、superseded でないもの
  ├ Test-WaitForSync         同期が拾うはずの通知は数分待つ (Invoke-Triage:112)
...
`Move-TaskEvent` (:664) が **`version` を上げない**のは意図的。
...
| 操作 | 関数 | 効き方 |
|---|---|---|
| 指示を書いて差し戻す | `Request-TaskRework` (:1247) | `todo` に戻す。ただし `doing` は動かさない (中止要求と区別が付かないため)、`setup` と アーカイブ済みも動かさない |
| 中止 | `Set-TaskCancel` (:1445) | フラグを立てるだけ。実際に止めるのはワーカーが各ステップ前に見る `Test-TaskCancelled` |
| 資格情報が入った | `Get-TasksWaitingForSetup` → `Resume-Task` (:852, :884) | 止まっていたカードを**まとめて** `todo` に戻す。`human_step` と `shape` は消す (もう事実ではない) |
...
- **WAL + `busy_timeout` 5 秒** (`lib/WinSqlite.ps1:134`)。読みが書きを止めない。
- **明示的なトランザクションは2か所だけ** ―― `Get-NextWorkItem` と `Remove-Tasks`/`Remove-Task`。
  ほかは単文で足り、`version` の照合が入っているものは単文のまま原子的になる。
- **一括削除は全件で1つのトランザクション** (:1206)。途中で外部キーに引っかかっても
  「何枚かだけ消えた」にはならず、押す前に戻る。
- **UI のポーリングは `Get-BoardRevision`** (:965)。カード・コメント・作業ログ・成果物・
...
| | |
|---|---|
| `$script:Schema` (:14) | **新規 DB の初期形**。`CREATE TABLE IF NOT EXISTS` の塊 |
| `Invoke-SchemaMigration` (:145) | **既存 DB への追随**。`PRAGMA table_info` で見て `ALTER TABLE` |

`Open-TaskStore` (:355) が毎回両方を流す。移行は冪等で、起動のたびに走ってよい。

> **`CREATE TABLE IF NOT EXISTS` は表が別の形で既にあると何もしない。**
> 開発途中の版 (`state` が無く `model` がある) で作られた `task_sessions` を持つ DB が実際にあり、
> 全カードの作業が `table task_sessions has no column named state` で落ちた (:337-352)。
> **表を足すときも、列は `PRAGMA` で見て足すこと。**
...
| 二つ目の Backlog を繋いだら何も入らない | お知らせ ID がスペース内連番で既存と衝突 | `ConvertTo-AccountSourceKey` (:373) |
| 全カードが `no column named state` で落ちる | 古い形の `task_sessions` に `CREATE IF NOT EXISTS` が効かない | 表にも `PRAGMA` 確認 (:341) |
| GET のつもりで押した「今後すべて許可」が PUT まで通した (#295) | 許可の単位がツール名だった | `http_request:{種類}:{ホスト}`。移行で旧 `http_request` の許可は読み取りに落として削除 (:313-317) |
| 差し戻すと前回の報告が読めない | 報告を `agent_output` に上書きしていた | `task_comments.kind = 'report'` に積む (:166-175) |
| 記録を保存すると送信欄に現れる | `user_edited` が送る文面と記録を兼ねていた | `user_record` を分離 (:157-164) |
| 送信後に点検が問題を見つけても他と同じ見た目 (#295) | 印が無かった | `tasks.alert`。利用者が確認するまで赤い (:236) |
| 設定カードが何枚も立つ | 束ねる鍵がモデルの自由記述だった | `subject_key = 'setup:{service}'` (:783) |
...
- **`Invoke-SchemaMigration` が 200 行の一本道** (:145-352)。冪等で順序に依存しないので
...
- **`memories` と `dossier` の表定義だけがここにあり、読み書きは `Memory.ps1` / `Dossier.ps1`**。
