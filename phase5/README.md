...
| ファイル | 役割 |
|---|---|
| `lib/SecretStore.ps1` | 資格情報を DPAPI で暗号化して保存。アカウントごとの名前空間もここ |
| `lib/AccountStore.ps1` | 同じ連携先に繋がっているアカウントの名簿 |
...
**片方だけ繋いだのでは、もう片方に届いたものはこのアプリを使っていないのと
同じところに戻る。**なので全アカウントを同じ扱いで掃き寄せる。

分かれるのは4つ。

| 何が | どう分かれるか |
|---|---|
| 資格情報 | `slack.userToken` → 2人目からは `slack.userToken#2` |
| watermark | `sync.slack.lastTs` → 2人目からは `sync.slack.lastTs#2` |
| イベントの主キー | `source_key` の頭にアカウント ID が付く (2人目から) |
| コネクタの控え | 切り替えのたびに落とす (トークン・自分のID・チャンネル名) |

**1人目だけは何も付けない。**既存の保管庫と DB がそのまま1人目として読めるので、
入れ替え作業が一度も要らない。名簿が無ければ「1人目だけがいる」と見なす。

主キーにアカウントを足すのは、`source_key` の一意性が効く範囲が経路によって
違うからである。Backlog のお知らせ ID は**スペース内の連番**なので、
付けないと二つ目のスペースを繋いだ瞬間に `UNIQUE(source, source_key)` に弾かれ、
「繋いだのに何も入らない」になる。

同じ用件が2つのアカウントに届いたとき (同じ Chatwork の部屋、同じメールが
二つの宛先に) は、既存の突き合わせ (`dedup_key` / `superseded_by`) がカードの段で
1枚に畳む。ここは作り直していない。

カードには**どのアカウントに届いたか**が出る (2つ以上繋いでいるときだけ)。
返信の名義が変わるので、カードだけ見て分からないのでは判断できない。
ワーカーは1枚の処理に入る手前で `Use-EventAccount` を呼び、以降の取り直しも
投稿先の解決もそのアカウントに固定する。

同意画面まわりの注意が1つある。**ブラウザに1つ目のセッションが残っていると、
2つ目を繋いだつもりで1つ目をもう一度繋ぐことになる。**画面上は成功するのに
カードが1枚も増えない、という一番読めない失敗の形なので、
Google には `prompt=select_account` を付け、どのサービスでも繋いだ直後に
アカウント名を突き合わせて、重複していればその場で弾く。

配る人が用意するアプリ登録 (`slack.clientId` / `gmail.clientId` / `ms.clientId` と
それぞれのシークレット) は**アカウントで分けない。**分けると2つ目を繋ぐときに
「配布時に設定済み」が効かなくなり、利用者の手では取れない値を空欄として
出すことになる。逆に `ms.tenantId` と `backlog.space` はアカウントごとに違うので分ける。

### 使い方

カンバンのヘッダの「接続」→ 各サービスの「別のアカウントを追加」。
枠が増えるので、いつもどおりそこに繋ぐ。呼び名 (「仕事」「個人」) を付けておくと
カードの隅に出る。外すのは2通りで、**切断**は資格情報だけ消して枠を残し、
**削除**は枠ごと消す (最後の1つは削除できない)。

端末からも同じことができる。

```powershell
.\phase5\Connect-Service.ps1 -Status                      # 番号と内訳を見る
.\phase5\Connect-Service.ps1 -Service gmail -AddAccount   # 枠を増やす
.\phase5\Connect-Service.ps1 -Service gmail -Account 2    # その枠に繋ぐ
```

**同期時間と API 消費はアカウント数に比例する。** `Sync-Sources.ps1` は直列なので、
3アカウントなら素朴に3倍かかる。`-SlackMax` などの上限はアカウントごとの意味になる。

## watermark 同期

`settings` テーブルに「前回どこまで取ったか」を持つ。アカウントを複数繋いでいる
場合は、2人目から `#<番号>` が付いた別のキーになる。
...
Claude の欄には任意の項目が1つある ―― **管理 API キー** (`sk-ant-admin...`)。
組織全体の口 (`/v1/organizations/...` 利用状況など) を叩かせるときだけ要る
（通常のキーでは通らない）。**これは組織の管理権限そのものなので配布設定には書けない。**
要る人だけがここから入れる。組織 ID は聞かない ―― どの組織かは貼ったキーが決めていて、
組織 ID を URL に載せる API は無い。
...
「どのアカウントとして繋がったか」しか返さない ―― アカウントを複数繋いでいても、
返るのは枠ごとの状態と表示名だけである)。

アカウントの枠を削除すると、その枠の資格情報も一緒に消す。
**トークンはメールボックスや会話の全履歴への鍵そのもの**なので、
使わなくなった時点で残さない。取り込み済みのカードは消えない。
...
Microsoft 365 のアプリ登録 (必要ならシークレット)、Claude の API キー ――
