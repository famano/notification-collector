...
Claude の欄には任意の項目が2つある ―― **組織 ID** と**管理 API キー**。
組織 ID はキーの組織を突き合わせるためと、URL に組織 ID が要る API を
ワーカーが組み立てるために使う。管理 API キー (`sk-ant-admin...`) は
`/v1/organizations/...` を叩かせるときだけ要る（通常のキーでは通らない）。
**これは組織の管理権限そのものなので配布設定には書けない。** 要る人だけがここから入れる。
